...
class Programm():
	def __init__(self,root):
		...
		self.list_box_2 = tk.Listbox(root)
		self.list_box_3 = tk.ttk.Treeview(root)
		...
		self.list_box_3.grid(row = 6, column = 2, sticky = tk.W + tk.E)
		
		self.nodes = dict()

		ysb = tk.ttk.Scrollbar(root, orient='vertical', command=self.list_box_3.yview)
		xsb = tk.ttk.Scrollbar(root, orient='horizontal', command=self.list_box_3.xview)
		abspath = os.path.abspath('.')


		self.list_box_3.configure(yscroll=ysb.set, xscroll=xsb.set)
		self.list_box_3.heading('#0', text='Project tree', anchor='w')



		ysb.grid(row=6, column=2, columnspan = 2, sticky='ns')
		xsb.grid(row=7, column=2, sticky='ew')
	...
	def insert_node(self,tree, parent, data):
		for key in data:
			uid = uuid.uuid4()
			if isinstance(data[key], dict):
				self.list_box_3.insert(parent, 'end', uid, text = key)
				self.insert_node(tree, uid, data[key])
			elif isinstance(data[key], list):
				self.list_box_3.insert(parent, 'end', uid, text = key + '[]')
				self.insert_node(self.list_box_3, uid, dict([(x,i) for i, x in enumerate(data[key])]))
			else:
				value = data[key]
				if value is None:
					value = 'None'
	...
	def send_POST(self):
		...
		self.insert_node(self.list_box_3, '', json_data)
...
class PlaceText:
	...
	def place(self):
		...
		try:
			...
			json_data = json.load(text_place)
